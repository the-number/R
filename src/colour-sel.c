...
gboolean
draw_texture_sample (GtkWidget * widget, GdkEventExpose * event,
		     gpointer data);
...
static gboolean
colour_select_response (GtkWidget * w, GtkResponseType resp,
			gpointer user_data);
...
createColourSelector (void)
{
  ...
  g_signal_connect (colourSelectorWidget, "color-changed",
		    G_CALLBACK (update_swatch_colours), colourSelectorWidget);
...
get_image_distr (void)
{
  ...
  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (button_tile),
				distr == TILED);
...
colour_select_menu (GtkWidget * w, GtkWindow *window)
{
...

  {
    ...
    g_signal_connect (button_mosaic, "toggled",
		      G_CALLBACK (set_image_distr), (gpointer) MOSAIC);
    ...
    g_signal_connect (button_use_image, "clicked",
		      G_CALLBACK (popupImageSelector), (gpointer) dialog);
  ...
  for (i = 0; i < 6; ++i)
	...
	{"application/x-color", 0, RDRAG_COLOUR},
      ...
      gtk_widget_set_tooltip_text (button,
				   _
				   ("A sample of the colour or pattern. You can click and select a new colour or pattern,  or drag one to this space."));

      swatch[i] = gtk_drawing_area_new ();

      gtk_widget_set_size_request (swatch[i], SWATCH_WIDTH, SWATCH_HEIGHT);
      ...
      g_signal_connect (button, "clicked",
			G_CALLBACK (select_side), (gpointer) i);

      g_signal_connect (button, "clicked",
			G_CALLBACK (preset_cs), colourSelectorWidget);
...
preset_cs (GtkButton * button, gpointer user_data)
{
  ...
  colour.green =
    proposed_face_rendering[selected_swatch].sr.color[1] * 65535.0;
  colour.blue =
    proposed_face_rendering[selected_swatch].sr.color[2] * 65535.0;
...
select_texture (GtkButton * button, gpointer user_data)
{
  ...
  if (-1 == i)
    ...
    {
      proposed_face_rendering[selected_swatch].rendering.texName =
	stock_pattern[i].texName;
    }
