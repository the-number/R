% 20220731 (C) Gunter Liszewski -*- mode: tex; -*-
% permutation basics
...
In terms of the set of permutation cycles the entaglement may read
$\{(\a FR RF ),(\a RF FR )\},$ and again we note that
$(\a FR RF )=(\a RF FR ).$}

\e The corner-cubes have a similar entanglement. (re-thing, re-word)
{\def\a{\hskip 3cm}%
\def\b{\hskip 24pt}%
\def\c#1 {{\bf #1 }}%
\vskip 12pt%
...
the mis-representation are creeping all over; however
we will wittle things down just to pick up each one
...
Given the standard permutaion $\sigma$ with
...
We want to utilize the $\backslash$halign
...
\vfil\eject
Now we parameterize the four edge cubes that we see
on this face.
...
Now, then! we backup a few steps and just move staight ahead.
...
this shared edge is wirtten as $50{}_4$ being on the
...
as a consequence of how our faces are numberd.
...
$\sigma_U^e=(21,25,20,24){}_U^e$, $\sigma_D^e=(30,35,31,34){}_D^e$,
...
is the axis from left to right. {\it(We admid that the
concept of direction such as left and right is a
woozy one; but it's not vacuous.)}

\e
\edges 21 25 20 24.\par\thinspace
\edges 02 05 03 04.\qquad\edges 50 52 51 53.\qquad
\edges 12 14 13 15.\qquad\edges 42 40 43 41.\par\thinspace
\edges 30 35 31 34.
...
$F_2$ sees $05\to 25$, $B_4$ moves $15\to 35$,
$U_2$ substitues $25\to 15$, and finally $D_2$
transposes its edge $35\to 05$. (to verify)
